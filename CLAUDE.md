...
`src/config.ts`'s `loadConfig()` fetches `/config.json` at boot (kicked off once, unawaited, from `main.jsx`, so it resolves in parallel with the splash screen) to get `AUTH_URL` / `API_URL` / `IDENTITY_API_URL`, falling back to `VITE_*` env vars for local dev (see `.env.example`). This lets one built Docker image be reused across environments: `docker-entrypoint.sh` regenerates `public/config.json` from the container's env vars at startup rather than baking URLs in at build time. If you add a new backend URL, thread it through `config.json` / `docker-entrypoint.sh` / `.env.example`, not `import.meta.env` alone.
